...
const createOrder = async (req, res) => {
	...
	for (const item of cartItems) {
		...
		// properties come from the ProductSchema
		const { name, price, image, _id } = dbProduct;

		// properties come from the SingleOrderItemSchema
	...
	const order = await Order.create({
		// properties come from the OrderSchema
		orderItems,
		total,
		subtotal,
		tax,
...
const getAllOrders = async (req, res) => {
	// rmb for admin only, so in routes, just make sure you authorise user
